 ...
 */
const ABOUT_INTRO = [
  // ⚠️ 2026-08-24 本人指定拿掉「幾乎」。原句是「幾乎都待在同一個地方」——
  //    那個保留語氣是寫的時候自己加的，本人這 8 年就是都在海線，加了反而把話講弱。
  //    不要再加回去。
  `入行邁入第 ${YEARS_IN_TRADE} 年，這 ${YEARS_IN_TRADE} 年我都待在同一個地方——台中海線。`,
  // ⚠️ 2026-08-24 本人指定拿掉「哪條路段將來會變」。
  //    那句等於對外宣稱知道未來的建設／重劃，跟「不准捏造建設、交通、政策」那條原則衝突。
  //    不要再加回去，也不要換成「哪裡會漲」這種同義的說法。
 ...
 *
 * ⚠️ role 是 2026-08-24 本人逐則確認的（換屋／首購／首購），**不是推測出來的**。
 *    要新增評價時，身份一樣要問過本人才能寫，不要從內容猜。
