...
import { TENANT_SHOWING_STATUSES, TENANT_SHOWING_INTENT_LEVELS, type TenantShowingInput, type TenantShowingRow } from "@/lib/tenant-showing";
import { createTenantShowingAction } from "@/lib/actions/tenant-showing-actions";
...
const STATUS_LABELS: Record<string, string> = Object.fromEntries(TENANT_SHOWING_STATUSES.map((s) => [s.key, s.label]));

export default function TenantShowingSection({ tenantId, showings }: { tenantId: string; showings: TenantShowingRow[] }) {
  ...
  const submit = async () => {
    ...
    const input: TenantShowingInput = {
      tenantId,
    ...
    const result = await createTenantShowingAction(
      input,
      status === "completed"
        ...
        <div style={{ borderTop: `1px solid ${CIS.cardBorder}`, paddingTop: 14, marginBottom: 16 }}>
          <div className={styles.formGrid}>
            ...
            <label className={styles.field} style={{ color: CIS.textMute }}>
              ...
              <select style={inputStyle} value={intentLevel} onChange={(e) => setIntentLevel(e.target.value)}>
                ...
                {TENANT_SHOWING_INTENT_LEVELS.map((l) => (
          ...
          {status === "completed" ? (
            <div style={{ marginTop: 14 }}>
              <div style={{ color: CIS.textMute, fontSize: 14, marginBottom: 7 }}>下次追蹤（會更新租客的下次追蹤時間）</div>
                ...
                <div className={styles.chips}>
                  ...
                  {s.intent_level ? (
                    <Chip tone={s.intent_level === "high" ? "warn" : s.intent_level === "low" ? "neutral" : "info"}>
                      意願{TENANT_SHOWING_INTENT_LEVELS.find((l) => l.key === s.intent_level)?.label || s.intent_level}
                    </Chip>
                  ) : null}
                </div>
              </div>
